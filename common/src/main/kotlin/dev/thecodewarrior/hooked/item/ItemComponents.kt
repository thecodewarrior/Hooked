...
object ItemComponents {
     ...
     */
    val HOOK_COUNT = register(Identifier.of(Hooked.MOD_ID, "hook_count"), Codec.INT)

    /**
     * The maximum range from impact point to player
     */
    val HOOK_RANGE = register(Identifier.of(Hooked.MOD_ID, "hook_range"), Codec.DOUBLE)

    /**
     * The speed of the fired hooks in m/t
     */
    val HOOK_SPEED = register(Identifier.of(Hooked.MOD_ID, "hook_speed"), Codec.DOUBLE)
    ...
    private fun <T> register(id: Identifier, codec: Codec<T>): ComponentType<T> {
        ...
        HookedRegistries.DATA_COMPONENT_TYPE.register(id) { componentType }
...
data class HookProperties(
    ...
    fun toNBT(): NbtElement {
        return CODEC.encodeStart(NbtOps.INSTANCE, this)
            .resultOrPartial { logger.error("Error encoding hook properties: $it") }
            .getOrElse {
                NONE.toNBT()
            }
    }
    ...
    companion object {
        fun fromNBT(tag: NbtElement): HookProperties {
            return CODEC.parse(NbtOps.INSTANCE, tag)
                .resultOrPartial { logger.error("Error decoding hook properties: $it") }
                .orElse(NONE)
        }
        ...
        val PACKET_CODEC = PacketCodecs.NBT_ELEMENT.xmap(::fromNBT, HookProperties::toNBT)
...
data class HookModelInfo(
    ...
    companion object {
        val DEFAULT = HookModelInfo(
            Identifier.of(Hooked.MOD_ID, "models/hook/base.obj"),
            Identifier.of(Hooked.MOD_ID, "textures/hook/base/hook.png"),
            0.5f
        )
...
data class ChainAppearance(
    ...
    companion object {
        val DEFAULT = ChainAppearance(
            Identifier.of(Hooked.MOD_ID, "textures/hook/base/chain1.png"),
            Identifier.of(Hooked.MOD_ID, "textures/hook/base/chain2.png"),
            0.0,
            Optional.empty(),
            Optional.empty(),
        )
