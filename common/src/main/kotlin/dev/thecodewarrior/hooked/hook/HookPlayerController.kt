...
abstract class HookPlayerController {
     ...
     */
    protected open fun writeSyncState(buf: PacketByteBuf, initial: Boolean) {}
    /**
     * Read from the synchronization packet.
     *
     * @param initial true if [buf] is the initial sync packet or false if [buf] a subsequent update packet
     */
    protected open fun readSyncState(buf: PacketByteBuf, initial: Boolean) {}

    fun writeSyncState(initial: Boolean): ByteArray {
        val packetBuf = PacketByteBuf(Unpooled.buffer())
        this.writeSyncState(packetBuf, initial)
        val array = ByteArray(packetBuf.readableBytes())
        packetBuf.readBytes(array)
        return array
    }

    fun readSyncState(state: ByteArray, initial: Boolean) {
        val packetBuf = PacketByteBuf(Unpooled.wrappedBuffer(state))
        this.readSyncState(packetBuf, initial)
    }
    ...
    open fun onHookMiss(delegate: HookControllerDelegate, hook: Hook) {
        delegate.playFeedbackSound(HookSounds.HOOK_MISS, 1f, 1f)
        spawnChainShatterParticleEffect(delegate, hook)
    }
    ...
    open fun onHookDislodge(delegate: HookControllerDelegate, hook: Hook, reason: DislodgeReason) {
        when(reason) {
            DislodgeReason.BLOCK_BROKEN, DislodgeReason.DISTANCE -> {
                delegate.playFeedbackSound(HookSounds.HOOK_DISLODGE, 1f, 1f)
            }
            ...
            DislodgeReason.EXPLICIT -> {
                delegate.playWorldSound(Hook.hitSound(delegate.world, hook.block), hook.pos, 1f, 1f)
                delegate.playFeedbackSound(HookSounds.RETRACT_HOOK, 1f, 1f)
            }
        }
        spawnChainShatterParticleEffect(delegate, hook)
    }
