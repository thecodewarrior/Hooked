...
object HookBehaviors {
    val BASIC = register("hooked:basic", HookBehaviorType(BasicHookBehavior.CODEC))
    val FLIGHT = register("hooked:flight", HookBehaviorType(FlightHookBehavior.CODEC))
    val NONE = register("hooked:none", HookBehaviorType(NullHookBehavior.CODEC))

    private fun <T : HookBehavior> register(id: String, type: HookBehaviorType<T>): HookBehaviorType<T> {
        return Registry.register(HookBehaviorType.REGISTRY, Identifier.of(id), type);
    }
}
